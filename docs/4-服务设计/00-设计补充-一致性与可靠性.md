...
### 1.2 解决方案：Redis-based Transactional Outbox

┌─────────────────────────────────────────────────────────────────────────────┐
│ Redis Outbox Pattern 架构 │
│ │
│ eidos-trading │
│ ┌────────────────────────────────────────────────────────────────────┐ │
│ │ CreateOrder (OrderService) │ │
│ │ │ │
│ │ 1. Redis Lua Script (Atomic) │ │
│ │ ├─ CHECK Nonce │ │
│ │ ├─ UPDATE Balance (Freeze) │ │
│ │ ├─ INSERT OrderFreeze Record │ │
│ │ └─ LPUSH Outbox List (消息入队) │ │
│ │ │ │
│ │ 2. DB Transaction (Sync) │ │
│ │ ├─ INSERT Orders │ │
│ │ └─ INSERT BalanceLog │ │
│ │ │ │
│ │ If DB fails: ROLLBACK Redis (Lua CancelPendingOrder) │ │
│ │ │ │
│ └────────────────────────────────────────────────────────────────────┘ │
│ │ │
│ ▼ │
│ ┌────────────────────────────────────────────────────────────────────┐ │
│ │ Order Outbox Relay (后台协程) │ │
│ │ │ │
│ │ RPOP Redis Outbox List: │ │
│ │ 1. 获取 OrderID │ │
│ │ 2. 读取 Outbox Hash (状态检查) │ │
│ │ 3. 发送到 Kafka │ │
│ │ 4. 更新 Outbox Hash status = 'SENT' │ │
│ │ │ │
│ └────────────────────────────────────────────────────────────────────┘ │
│ │ │
│ ▼ │
...
| Key 类型         | Key Pattern                                     | 描述                                  |
| :--------------- | :---------------------------------------------- | :------------------------------------ |
| **Balance Hash** | `eidos:trading:balance:{wallet}:{token}`        | 用户余额 (settled/pending read model) |
| **Order Freeze** | `eidos:trading:freeze:order:{order_id}`         | 订单冻结明细 (Amount, Token)          |
| **Outbox Hash**  | `eidos:trading:outbox:order:{order_id}`         | Outbox 消息详情 (Status, Payload)     |
| **Outbox List**  | `eidos:trading:outbox:order:pending:{shard_id}` | 待发送消息队列 (FIFO)                 |
| **Nonce Lock**   | `nonce:{wallet}:order:{nonce}`                  | 幂等性控制 (SETNX/Expiration)         |
...
```go
// OrderOutboxRelay 负责消费 Redis Queue 并发送 Kafka
type OrderOutboxRelay struct {
    rdb      redis.UniversalClient
    producer *kafka.Producer
}

func (r *OrderOutboxRelay) relayLoop(shardID int) {
    for {
        // RPOP 阻塞式获取或者轮询
        orderID, err := r.rdb.RPop(ctx, pendingKey).Result()

        // 获取消息详情
        outboxData := r.rdb.HGetAll(ctx, outboxKey)
        ...
        if err == nil {
            // 更新状态 SENT
            r.rdb.HSet(ctx, outboxKey, "status", "SENT")
        } else {
            // 失败重试逻辑 (LPUSH back or Dead Letter)
        }
    }
}
```
...
func (s *OrderService) CreateOrder(ctx context.Context, req *CreateOrderRequest) error {
    // 1. Redis 原子操作 (Lua Script)
    // - 检查 Nonce
    // - 冻结余额 (FreezeForOrder)
    // - 写入 Outbox List (PENDING)
    err := s.balanceCache.FreezeForOrder(ctx, freezeReq)
    if err != nil {
        return err // 余额不足或风控拒绝
    }
    ...
    // 3. 错误处理与回滚
    if err != nil {
        // 严重错误：Redis 成功但 DB 失败
        // 必须回滚 Redis 状态，否则会导致"幽灵订单" (资金冻结了但没订单)
        s.balanceCache.CancelPendingOrder(ctx, rollbackReq)
...
func (s *Snowflake) Generate() (int64, error) {
    ...
    return (now << timestampShift) | (s.machineID << machineIDShift) | s.sequence, nil
}

// MustGenerate 生成 ID，时钟回拨时 panic (用于不允许失败的场景)
...
| 回拨幅度 | 处理方式 | 说明 |
|----------|----------|------|
| <= 5ms | 等待追上 | 短暂回拨，等待时钟恢复后继续 |
| > 5ms | 拒绝生成 | 返回错误，由调用方决定重试或降级 |

**调用方处理建议**:

```go
func (s *OrderService) CreateOrder(ctx context.Context, req *CreateOrderRequest) (*Order, error) {
    orderID, err := s.idGen.Generate()
    if err != nil {
        if errors.Is(err, idgen.ErrClockMovedBackwards) {
            // 记录告警，时钟回拨可能影响系统
            alert.Send(alert.Warning, "Clock moved backwards", "service", "trading")
            // 使用 UUID 作为降级方案
            orderID = uuid.New().ID()
        } else {
            return nil, err
