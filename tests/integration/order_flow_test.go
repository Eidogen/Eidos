...
// simulateDeposit simulates a deposit by directly setting Redis balance
func (s *OrderFlowTestSuite) simulateDeposit(wallet, token, amount string) {
	err := s.helper.SetBalance(s.ctx, wallet, token, amount)
	if err != nil {
		s.T().Logf("Warning: Failed to set balance: %v", err)
	}
}
...
func (s *OrderFlowTestSuite) TestCreateLimitOrder() {
	...
	// Deposit USDT for buying ETH
	s.simulateDeposit(wallet, s.config.TestQuoteToken, "10000.00")
...
func (s *OrderFlowTestSuite) TestCreateMarketOrder() {
	...
	s.simulateDeposit(wallet, s.config.TestBaseToken, "10.0")
	nonce := GenerateNonce()
	...
	req := &tradingv1.CreateOrderRequest{
		...
		Price:       "0", // Market orders use 0 price
		Amount:      "0.5",
		Nonce:       nonce,
		Signature:   MockSignature(),
...
func (s *OrderFlowTestSuite) TestOrderMatching() {
	...
	// Deposit ETH for selling
	s.simulateDeposit(makerWallet, s.config.TestBaseToken, "10.0")
	...
	s.simulateDeposit(takerWallet, s.config.TestQuoteToken, "10000.00")
...
func (s *OrderFlowTestSuite) TestCancelOrder() {
	...
	// Wait for order to be processed by matching engine
	time.Sleep(500 * time.Millisecond)

	// Cancel the order - may fail if order is already in matching engine
	_, err = s.tradingClient.CancelOrder(s.ctx, &tradingv1.CancelOrderRequest{
		Wallet:  wallet,
		OrderId: orderID,
	})
	// Accept either success or "order_already_sent" error (order in matching engine)
	if err != nil {
		s.T().Logf("Cancel order result: %v (expected if order already in matching engine)", err)
	}

	// Verify order status changed (cancelled or still open in matching engine)
	order, err := s.tradingClient.GetOrder(s.ctx, &tradingv1.GetOrderRequest{
		OrderId: orderID,
	})
	require.NoError(s.T(), err)
	// Order should exist regardless of cancel result
...
func (s *OrderFlowTestSuite) TestBatchCancelOrders() {
	...
	if err != nil {
		...
		s.T().Skipf("BatchCancelOrders not implemented: %v", err)
		return
	}
