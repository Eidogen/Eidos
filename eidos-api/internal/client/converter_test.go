...
func TestConvertOrderSideToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertOrderSideToString(pb.OrderSide(tt.input))
...
func TestConvertOrderTypeToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertOrderTypeToString(pb.OrderType(tt.input))
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestConvertTimeInForceToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertTimeInForceToString(pb.TimeInForce(tt.input))
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestConvertOrderStatusToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertOrderStatusToString(pb.OrderStatus(tt.input))
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestConvertBalanceLogType(t *testing.T) {
	tests := []struct {
		input string
		want  int32
	}{
		{"deposit", 1},         // BALANCE_LOG_TYPE_DEPOSIT
		{"DEPOSIT", 1},         // case insensitive
		{"withdraw", 2},        // BALANCE_LOG_TYPE_WITHDRAW
		{"freeze", 3},          // BALANCE_LOG_TYPE_FREEZE
		{"unfreeze", 4},        // BALANCE_LOG_TYPE_UNFREEZE
		{"trade", 5},           // BALANCE_LOG_TYPE_TRADE
		{"fee", 6},             // BALANCE_LOG_TYPE_FEE
		{"settlement", 7},      // BALANCE_LOG_TYPE_SETTLEMENT
		{"rollback", 8},        // BALANCE_LOG_TYPE_ROLLBACK
		{"withdraw_refund", 9}, // BALANCE_LOG_TYPE_WITHDRAW_REFUND
		{"invalid", 0},         // BALANCE_LOG_TYPE_UNSPECIFIED
		{"", 0},                // empty
	}

	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			got := convertBalanceLogType(tt.input)
			assert.Equal(t, tt.want, int32(got))
		})
	}
}

func TestConvertBalanceLogTypeToString(t *testing.T) {
	tests := []struct {
		input int32
		want  string
	}{
		{1, "deposit"},         // BALANCE_LOG_TYPE_DEPOSIT
		{2, "withdraw"},        // BALANCE_LOG_TYPE_WITHDRAW
		{3, "freeze"},          // BALANCE_LOG_TYPE_FREEZE
		{4, "unfreeze"},        // BALANCE_LOG_TYPE_UNFREEZE
		{5, "trade"},           // BALANCE_LOG_TYPE_TRADE
		{6, "fee"},             // BALANCE_LOG_TYPE_FEE
		{7, "settlement"},      // BALANCE_LOG_TYPE_SETTLEMENT
		{8, "rollback"},        // BALANCE_LOG_TYPE_ROLLBACK
		{9, "withdraw_refund"}, // BALANCE_LOG_TYPE_WITHDRAW_REFUND
		{0, "unknown"},         // BALANCE_LOG_TYPE_UNSPECIFIED
		{99, "unknown"},        // unknown value
	}

	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertBalanceLogTypeToString(pb.BalanceLogType(tt.input))
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestConvertDepositStatusToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertDepositStatusToString(pb.DepositStatus(tt.input))
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestConvertWithdrawStatusToString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertWithdrawStatusToString(pb.WithdrawStatus(tt.input))
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestConvertSettlementStatusToString(t *testing.T) {
	tests := []struct {
		...
		{4, "settled_onchain"},  // SETTLEMENT_STATUS_SETTLED_ONCHAIN
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := convertSettlementStatusToString(pb.SettlementStatus(tt.input))
			assert.Equal(t, tt.want, got)
		})
	}
}
...
func TestConvertProtoBalanceLog_Trade(t *testing.T) {
	log := &pb.BalanceLog{
		Id:           123,
		Wallet:       "0x1234",
		Token:        "USDC",
		Type:         pb.BalanceLogType_BALANCE_LOG_TYPE_TRADE,
		Amount:       "100.00",
		BalanceAfter: "900.00",
		TradeId:      "trade-123",
...
func TestConvertProtoBalanceLog_Fee(t *testing.T) {
	log := &pb.BalanceLog{
		...
		Wallet:       "0x1234",
		Token:        "USDC",
		Type:         pb.BalanceLogType_BALANCE_LOG_TYPE_FEE,
		Amount:       "-0.50",
...
func TestConvertProtoBalanceLog_Freeze(t *testing.T) {
	log := &pb.BalanceLog{
		...
		Wallet:       "0x1234",
		Token:        "USDC",
		Type:         pb.BalanceLogType_BALANCE_LOG_TYPE_FREEZE,
		Amount:       "100.00",
		BalanceAfter: "800.00",
		OrderId:      "order-123",
		CreatedAt:    1704067200000,
...
func TestConvertProtoBalanceLog_Unfreeze(t *testing.T) {
	log := &pb.BalanceLog{
		...
		Wallet:       "0x1234",
		Token:        "USDC",
		Type:         pb.BalanceLogType_BALANCE_LOG_TYPE_UNFREEZE,
		Amount:       "50.00",
		BalanceAfter: "850.00",
		OrderId:      "order-456",
		CreatedAt:    1704067200000,
...
func TestConvertProtoBalanceLog_Deposit(t *testing.T) {
	log := &pb.BalanceLog{
		...
		Wallet:       "0x1234",
		Token:        "USDC",
		Type:         pb.BalanceLogType_BALANCE_LOG_TYPE_DEPOSIT,
		Amount:       "500.00",
...
func TestConvertProtoBalanceLog_Withdraw(t *testing.T) {
	log := &pb.BalanceLog{
		...
		Wallet:       "0x1234",
		Token:        "USDC",
		Type:         pb.BalanceLogType_BALANCE_LOG_TYPE_WITHDRAW,
		Amount:       "-200.00",
...
func TestConvertProtoBalanceLog_WithdrawRefund(t *testing.T) {
	log := &pb.BalanceLog{
		...
		Wallet:       "0x1234",
		Token:        "USDC",
		Type:         pb.BalanceLogType_BALANCE_LOG_TYPE_WITHDRAW_REFUND,
		Amount:       "100.00",
...
func TestConvertProtoTrade(t *testing.T) {
	trade := &pb.Trade{
		...
		TakerFee:         "21.00",
		FeeToken:         "USDC",
		MakerSide:        pb.OrderSide_ORDER_SIDE_SELL,
		SettlementStatus: pb.SettlementStatus_SETTLEMENT_STATUS_SETTLED_ONCHAIN,
...
func TestConvertProtoDeposit(t *testing.T) {
	deposit := &pb.Deposit{
		...
		LogIndex:    5,
		BlockNum:    12345678,
		Status:      pb.DepositStatus_DEPOSIT_STATUS_CREDITED,
...
func TestConvertProtoWithdrawal(t *testing.T) {
	withdrawal := &pb.Withdrawal{
		...
		ToAddress:    "0x5678",
		Nonce:        12345,
		Status:       pb.WithdrawStatus_WITHDRAW_STATUS_CONFIRMED,
		TxHash:       "0xdef456",
		RejectReason: "",
